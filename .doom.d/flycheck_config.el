...
;; Add keybind for toggling window height
(defun create-window-height-toggler ()
  (setq! on-height nil
         off-height nil
         is-on nil
         )
  (lambda ()
    ...
    (setq toggle (lambda ()
                   (if is-on
                       (evil-window-set-height off-height)
                     (evil-window-set-height on-height)
                     )
                   (setq! is-on (not is-on))
                   ))
    (if on-height (funcall toggle)
      (setq! on-height (* 2 (window-total-height)) off-height (window-total-height))
      (funcall toggle)
      )))

(defun window-height-toggler ()
  (interactive)
  (defun get-window-toggler ()
    (lexical-let
        ((window-togglers '()) (selected-window (selected-window)))

      (lexical-let ((window-toggler (assoc selected-window window-togglers)))
        (if (not window-toggler)
            (lexical-let (
                          (on-height (* 2 (window-total-height selected-window)))
                          (off-height (window-total-height selected-window))
                          (is-on nil)
                          )
              (lexical-let ((new-toggler
                             (lambda ()
                               (if is-on
                                   (evil-window-set-height off-height)
                                 (evil-window-set-height on-height)
                                 )
                               (setf is-on (not is-on))
                               )
                             ))
                (push (list
                       selected-window new-toggler
                       ) window-togglers)
                (car (cdr (assoc selected-window window-togglers))))
              )
          (car (cdr window-toggler)))
        )
      ))

  (funcall (get-window-toggler))
  )

(map! :leader
      :desc "Toggle max/min window height"
      "w f" #'window-height-toggler
      )
...
;; Work around issues with fish and emacs compatibility
